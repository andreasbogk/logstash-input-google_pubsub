...
#
#         # The topic name is customizable here. You must first create this
#         # topic by hand and ensure you are exporting logging to this pubsub
#         # topic.
#         topic => "logstash-input-dev"
#
#         # The subscription name is customizeable here. The plugin will attempt
#         # to create the subscription (but use the hard-coded topic name above).
...
class LogStash::Inputs::GooglePubSub < LogStash::Inputs::Base
  ...
  # specify a Service Account JSON key. You can do this either directly,
  # or my specifying a key file. Key takes precedence over key file.
  config :json_key, :validate => :string, :required => false
  ...
  def register
    ...
    if @json_key
      @credentialsProvider = FixedCredentialsProvider.create(
        ServiceAccountCredentials.fromStream(java.io.StringBufferInputStream.new(@json_key))
      )
    end
